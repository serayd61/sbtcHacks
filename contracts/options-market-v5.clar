;; options-market-v5.clar
;; Marketplace for covered call options - v5 with 100K listings per epoch + batch creation
...
(define-constant MAX-LISTINGS-PER-EPOCH u100000)
...
;; Track listing count per epoch (max 100000)
...
(define-public (create-listing
  ...
  (begin
    ...
    ;; Allow up to MAX-LISTINGS-PER-EPOCH (100000) listings per epoch
    (let (
...
;; num-listings: how many listings to create (1-100 per call, can call multiple times)
(define-public (batch-create-listings
  ...
  (begin
    ...
    (asserts! (<= num-listings u100) ERR-INVALID-COUNT)
    (let (
      (current-epoch-count (default-to u0 (map-get? epoch-listing-count epoch-id)))
    )
