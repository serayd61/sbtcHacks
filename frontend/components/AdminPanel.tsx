...
import {
  ...
  buildSettleEpochTx,
  buildSetPriceTx,
  buildSetLogicContractTx,
  buildSetMarketContractTx,
...
export default function AdminPanel({ address, onTxComplete }: AdminPanelProps) {
  ...
  const handleSetLogicContract = () =>
    exec("setLogic", async () => {
      const { openContractCall } = await import("@stacks/connect");
      await openContractCall({
        ...buildSetLogicContractTx(),
        onFinish: (d) => {
          showToast("Logic contract set on vault-data-v1!", "success", d.txId);
          setTimeout(onTxComplete, 3000);
        },
        onCancel: () => showToast("Cancelled", "info"),
      });
  ...
  const handleSetMarketContract = () =>
    exec("setMarket", async () => {
      const { openContractCall } = await import("@stacks/connect");
      await openContractCall({
        ...buildSetMarketContractTx(),
        onFinish: (d) => {
          showToast("Market contract set on vault-logic-v2!", "success", d.txId);
          setTimeout(onTxComplete, 3000);
        },
        onCancel: () => showToast("Cancelled", "info"),
      });
  ...
  return (
    <div className="space-y-5">
          ...
          <AdminSection title="0. Initial Setup (one-time)" color="green">
            <p className="text-xs text-gray-500 mb-2">
              Required once after deployment to link contracts together.
            </p>
            <div className="flex gap-2">
              <ActionButton onClick={handleSetLogicContract} disabled={!address} loading={pending === "setLogic"} color="green">
                Set Logic Contract
              </ActionButton>
              <ActionButton onClick={handleSetMarketContract} disabled={!address} loading={pending === "setMarket"} color="green">
                Set Market Contract
              </ActionButton>
            </div>
          </AdminSection>
